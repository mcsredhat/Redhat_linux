...
configure_zones() {
    ...
    # Configure forward zone for linuxcbtserv2
    echo "Configuring forward zone for linuxcbtserv2..."
    if ! grep -q "linuxcbt.external" /etc/named.conf; then
...
EOF
    ...
    # Configure reverse zone for linuxcbtserv2
    echo "Configuring reverse zone for linuxcbtserv2..."
...
"the rest of ip address first octet take" IN PTR linuxcbtserv2.linuxcbt.internal.
EOF
    fi
}
...
sudo named-checkzone $reverse_zone /var/named/${reverse_zone}.zone

sudo systemctl restart named.service
echo "BIND configuration for linuxcbtserv2 complete."
